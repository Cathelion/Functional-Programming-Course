...
var, num, factor, term, power, expr :: Parser Expr
...
-- exponentiation
power' e = powerOp # factor >-> bldOp e #> power' ! return e
power = factor #> power' 
...
value (Div e1 e2) dict = if denom /=0 then (value e1 dict) `div` denom else error "Expr.value: Division by zero" where denom = value e2 dict
value (Power (Power e1 e2) e3) dict = value (Power e1 (Power e2 e3)) dict
value (Power e1 e2) dict = (value e1 dict) ^ (value e2 dict)
